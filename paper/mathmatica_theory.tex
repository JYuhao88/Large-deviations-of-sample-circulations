...
\begin{proof}
    ...
    \begin{align*}
        ...
        &+ \left(\sum_{c \in \mathcal{C}_{\infty}, c\neq (i)} \nu_{c} \log \frac{\nu_{c}}{\tilde{\nu} - \sum_{i\in S}\nu_i}
        -(\tilde{\nu} - \sum_{i\in S}\nu_i)\log(\frac{ w_c}{\tilde{w} - \sum_{i\in S}w_i}) \right)
    \end{align*}
    Obviously, $I_3^c(\alpha \nu) = \alpha I_3^c( \nu)$, so $I_3^c$ is also affine.
    The item 
    % $\nu^i \log \frac{w_i}{w^i}$, $\nu^i \log \frac{w^i - w_i}{w^i}$ and $(\tilde{\nu} - \sum_{i\in S}\nu_i)\log(\frac{ w_c}{\tilde{w} - \sum_{i\in S}w_i})$ 
    involving $w$
...
\end{proof}
\begin{corollary}
    $I_{m'}^{c}(\nu)$ is  finite, continuous, positive and strictly convex on E, except along line segments $\{\alpha \nu + (1-\alpha) \mu: \alpha \in [0,1]\}$, between any $\nu$ and $\mu$ satisfying $\nu_c/\nu^i = \mu_c/\mu^i, \forall c \ni i$. $I_{m'}^{c}(\nu)$ is also affine, i.e. $I_{m'}^{c}(\alpha \nu) = \alpha I_{m'}^{c}(\nu)$. convex. The proof can be obtained by completely imitating it for $I_{m'}^{c}(\nu)$.
\end{corollary}
% \begin{proof}
