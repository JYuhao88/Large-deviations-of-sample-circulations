...
\begin{proof}
    ...
    \begin{align*}
        ...
        \ge \mathcal{E} (G^m(k)) \Pi_{i,j} p_{ij}^{\sum_{c \ni [i,j]} k_{c}}
        \min_{\{i, j\}} p_{ij}^{m-1}
    \end{align*}
    ...
    \end{align*}
    and this set contains all possible situations of each cycles occuring amount. It can easily observe that the size of this set $|K_n| \le n^m$, $\frac{1}{n} K_n \in E$. 
...
\begin{proof}
    ...
    Let 
    \begin{align*}
        K_n = \biggl\{k=(k_c)_{c\in \mathcal{C}_{\infty}}: \sum_{c \in \mathcal{C}_{\infty}} k_{c} |c| = n \biggr\},
    \end{align*}
    and
    \begin{align*}
        Q_n(a) = \max_{k\in K_n: \mu_n(k) \in B_a(\nu)} 
