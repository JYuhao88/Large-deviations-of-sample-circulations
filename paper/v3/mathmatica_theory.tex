...
\section{Monocyclic markov chains and cycle fluxes}
...
\section{Large deviation of circulation for Monocyclic Markov chains}
...
\begin{proof}
    ...
    \begin{align*}
        I_m^c(\nu) = \frac{1}{n} \log \mathcal{E}_1 (G^m(k)) \Pi_{i, j} p_{ij}^{\sum_{c \ni [i,j]} k_{c}}.
    \end{align*}
    The accumulation part in $\mathcal{E}_1 (G^m(k))$ has no more than $n^{m-1}$ items, and $\frac{1}{n} \log n^{m-1} = O(\frac{\log n}{n})$. The other means for simplification has been mentioned in  Theorem 1.1 many times.
\end{proof}
...
\section{Appendix}
Consider the first n-step of the above Markov chains $(\xi_l)$, we assume that $\xi_{n}=\xi_1$. Let $k=(k_c)_{c \in \mathcal{C}_{\infty}}$ denote the number of each cycle in class  $\mathcal{C}_{\infty}$ occurring.
If marking each occurence of state pair $(s, t)$ in $\xi_1, \dots, \xi_n$ by drawing an arrow from $s$ to $t$, we can obtain an oriented graph. Hence if we know the number of all state pair $(s, t)$, the oriented graph is unique. Because we have known the amount of cycle $k=(k_c)_{c \in \mathcal{C}_{\infty}}$, the amount of each state pair can be denoted easily by $k$,  let $G^m(k)$ denote the corresponding oriented graph.
And we employ $\mathcal{E} (G^m(k))$ to denote the number of Euler circuits on $G^m(k)$ and which the cycle amount is $k$. By the way, $\mathcal{E}_i (G^m(k))$ denotes the number of Euler circuits which start from states $i$, i.e. $\xi_1 = i$.
